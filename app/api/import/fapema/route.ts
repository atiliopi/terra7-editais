...
function extractFapemaLinks(html: string) {
  ...
  for (const match of matches) {
    ...
    if (!href.includes("fapema.br/")) continue;
    if (!href.includes("edital")) continue;
...
Extraia dados de um edital da FAPEMA.
...
Regras:
- Não invente dados.
- A FAPEMA é do Maranhão. Use "Maranhão" apenas se fizer sentido pelo texto.
- Se não souber valor, use null.
- Se não souber datas, use null.
- Se faltar informação, registre em notes.
- Áreas prováveis: Pesquisa, Ciência, Tecnologia, Inovação, Educação, Empreendedorismo.
  ...
  try {
    ...
    if (!listResponse.ok) {
      return NextResponse.json(
        { error: "Não foi possível acessar a lista da FAPEMA." },
        { status: 500 }
      );
    }

    const listHtml = await listResponse.text();
    const links: FapemaGrant[] = extractFapemaLinks(listHtml);
    ...
    for (const item of links) {
      ...
      try {
        ...
        const { error: insertError } = await supabase.from("grants").insert([
          {
            code,
            title,
            state_scope: ai.state_scope || "Maranhão",
        ...
        console.error("ERRO_IMPORTAR_FAPEMA_ITEM:", error);
        errors++;
      }
    }
    ...
    return NextResponse.json({
      ...
      errors,
      details,
      message: `FAPEMA: ${imported} importados, ${duplicates} duplicados, ${ignoredClosed} encerrados ignorados, ${errors} erros.`,
    });
  } catch (error) {
    console.error("ERRO_IMPORTAR_FAPEMA:", error);

    return NextResponse.json(
      { error: "Erro interno ao importar editais da FAPEMA." },
      { status: 500 }
    );
  }
}